...
nvcc detect.cu -std=c++11 -O3 -Iinclude -Itools/marvin -I/usr/local/cuda/include -I/usr/local/cudnn/v4rc/include -Llib -lm -lpng -L$CUDA_LIB_DIR -L$CUDNN_LIB_DIR -lcurand -lcudart -lcublas -lcudnn `libpng-config --cflags` `pkg-config --cflags opencv` `pkg-config --libs opencv` -o detect
