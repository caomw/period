...
g++ train.cpp -std=c++11 -Iinclude -Llib -lm -lpng `libpng-config --cflags` `pkg-config --cflags opencv` `pkg-config --libs opencv` -o train
g++ test.cpp -std=c++11 -Iinclude -Llib -lm -lpng `libpng-config --cflags` `pkg-config --cflags opencv` `pkg-config --libs opencv` -o test
